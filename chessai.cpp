...
chessAi::POINTS chessAi::seekPoints(int board[15][15]){
    bool B[15][15];//标记数组
    int worth[15][15];
    chessAi::POINTS best_points;
...
int chessAi::analyse(int (*board)[15], int depth,int alpha, int beta){
    ...
    if(depth==0||EVAL.result!=R_DRAW){//抵达最深层/如果模拟落子可以分出输赢，那么直接返回结果，不需要再搜索
        ...
        chessAi::POINTS P=seekPoints(board);
        ...
        chessAi::POINTS P=seekPoints(rBoard);//找对于黑子的最佳位置,需要将棋盘不同颜色反转,因为seekPoint是求白色方的最佳位置
...
QList<QPoint> chessAi::seek_kill_points(int (*board)[15]){
    ...
    chessAi::POINTS P=seekPoints(board);//一般来说,能冲4或者活3的必在评分前20的点内
